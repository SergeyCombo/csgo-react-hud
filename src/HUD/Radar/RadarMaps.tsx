        ...
        return (
            <div id={`radar_maps_container`} className={`${!showRadar ? 'hide' : ''} ${showBig ? 'preview' : ''}`}>
                <div className="radar-component-container" style={{ width: `${size}px`, height: `${size}px` }}>
                    <Radar radarSize={size} game={this.props.game}/></div>
                {
                    match
        ...
        if (picks.length > 3) {
            ...
            return <div id="maps_container">
                {<MapEntry veto={current} map={map}
                           team={current.type === "decider" ? null : map.team_ct.id === current.teamId ? map.team_ct : map.team_t}/>}
            </div>;
        }
        return <div id="maps_container">
            {match.vetos.filter(veto => veto.type !== "ban").filter(veto => veto.teamId || veto.type === "decider").map(veto =>
                <MapEntry key={veto.mapName} veto={veto} map={this.props.map}
...
class MapEntry extends React.PureComponent<{ veto: Veto, map: Map, team: Team | null }> {
    render() {
        ...
        return <div className="veto_entry">
            ...
            <div className={`map_name ${map.name.includes(veto.mapName) ? 'active' : ''}`}>{cleanMapName}</div>
        </div>;
    }
}